...
def capture_images(request):
    ...
    Captures and validates MULTIPLE images. No authentication required.
    """
    ...
    try:
        ...
        if not images:
            return error_response(
                code="MISSING_IMAGE",
                message="The 'imagen' field is required and must contain at least one file.",
        ...
        for image in images:
            serializer_data = {'imagen': image}
            
            # Pasar la instancia completa de current_user al save() del serializador
            # Si el usuario no está autenticado, request.user es AnonymousUser
            # Esto funcionará si el campo 'usuario' en el modelo ImagenReconocida es nullable.
            serializer = CapturedImageSerializer(data=serializer_data, context={'request': request})
            
            if serializer.is_valid():
                # Guardamos la imagen, asignando el usuario. Si 'current_user' es AnonymousUser,
                # y el campo 'usuario' del modelo es null=True, se guardará como NULL en la DB.
...
def get_multimedia_by_tag(request, tag):
    ...
    try:
        ...
        if not clean_tag:
            return error_response(
                code="INVALID_TAG",
                message="The tag contains invalid characters or is empty.",
        ...
        return error_response(
            code="MULTIMEDIA_SEARCH_ERROR",
            message="Failed to search multimedia resources by tag",
            details=str(e) if settings.DEBUG else "",
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def get_items(request):
    ...
    try:
        ...
        return error_response(
            ...
            message="Failed to retrieve the list of items",
            details=str(e) if settings.DEBUG else "",
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def get_item_detail(request, item_id):
    ...
    try:
        ...
        response_data = {
            ...
            "statistics": {
                "usages": 15 * item_id,
                "accuracy": max(0.01, 0.9 - (item_id * 0.01))
            }
        }
    ...
    try:
        ...
        if from_date_str:
            try:
                ...
                return error_response(
                    ...
                    message="The 'from_date' format must be YYYY-MM-DD.",
                    status_code=status.HTTP_400_BAD_REQUEST
                )
        ...
        if request.user.is_authenticated:
            ...
            results = [
                ...
                if item['user'] == current_username and
                (not from_date or item['date'] >= from_date)
            ]
        else:
            results = [
                item for item in HISTORY_DB
                if (not from_date or item['date'] >= from_date)
            ]
        ...
        return error_response(
            ...
            message="Failed to retrieve recognition history",
            details=str(e) if settings.DEBUG else "",
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
