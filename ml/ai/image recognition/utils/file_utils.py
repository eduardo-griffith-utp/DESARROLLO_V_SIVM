...
def list_image_files(directory, extensions=('jpg', 'jpeg', 'png')): 
    ...
    files.extend(glob.glob(pattern))
    return sorted(files) 
def create_train_val_split(image_dir, output_dir, val_ratio=0.2):
    """ 
    Crea una división entre entrenamiento y validación. 
    Args: 
        image_dir (str): Directorio con las imágenes originales. 
        output_dir (str): Directorio donde crear la estructura train/
        val_ratio (float): Proporción para validación (0.0-1.0). 
        copy (bool): Si es True, copia los archivos en lugar de mover
    Returns: 
        tuple: (Número de imágenes de entrenamiento, número de imágen
    """ 
    ...
    for img in train_images: 
            dst = os.path.join(train_dir, category, os.path.basename)
            if copy: 
    ...
    for img in val_images: 
            dst= os.path.join(val_dir, category, os.path.basename(im))
            if copy: 
                shutil.copy2(img, dst) 
    else: 
                shutil.move(img, dst) 
                val_count += len(val_images) 
    print(f"División completada: {train_count} imágenes de entrenamie")
